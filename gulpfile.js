// generated on 2018-01-17 using generator-webapp 2.4.1
...
gulp.task('cleanDev', del.bind(null, ['.tmp']));

gulp.task('rep', function(){
  var manifest = gulp.src('app/rev/*.json');
...
gulp.task('serve', () => {
  runSequence(['cleanDev', 'wiredep'], ['styles', 'scripts', 'fonts'], () => {
    browserSync.init({
      notify: false,
      port: 9000,
      server: {
...
gulp.task('default', () => {
  return new Promise(resolve => {
    dev = false;
    runSequence(['clean', 'wiredep'], 'build', (list)=>{
      runSequence('rep');
      resolve(list)
    });
  });
